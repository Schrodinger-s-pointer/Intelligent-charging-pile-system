...
*/
          ...
          break;
        case CMD_SERIAL_MSG:
          //printf("recv uart data\n");
          //HalUARTWrite (0,&((mtOSALSerialData_t *)MSGpkt)->msg[1],((mtOSALSerialData_t *)MSGpkt)->msg[0]);
...
static void rxCB(uint8 port, uint8 event)
{
  ...
  UART_len =  Hal_UART_RxBufLen(0); //先读取串口0 长度
    if(UART_len)  //有消息进入这里
    {
      osal_memset(uartbuf,'\0',strlen(uartbuf));   //将UART_RX内容清空
      HalUARTRead(0,uartbuf,UART_len); //读取消息
      //HalUARTWrite(0,uartbuf,UART_len);//发送消息
      SerialData_Analysis(uartbuf);
...
AF_DataRequest( &SampleApp_Periodic_DstAddr, &SampleApp_epDesc,
                       SAMPLEAPP_PERIODIC_CLUSTERID,
                       strlen(msg),
                       msg,//(uint8*)&SampleAppPeriodicCounter,
                       &SampleApp_TransID,
                       AF_DISCV_ROUTE,
                       AF_DEFAULT_RADIUS );
}
...
void SampleApp_SendPeriodicMessage( void )
{
  ...
  HalUARTWrite (0,buff,strlen(buff));
  if ( AF_DataRequest( &SampleApp_Periodic_DstAddr, &SampleApp_epDesc,
                       SAMPLEAPP_PERIODIC_CLUSTERID,
                       strlen(buff),
                       buff,//(uint8*)&SampleAppPeriodicCounter,
                       &SampleApp_TransID,
                       AF_DISCV_ROUTE,
